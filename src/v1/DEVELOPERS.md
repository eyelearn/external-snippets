...
1. Erzeuge ein JWT. Siehe https://datatracker.ietf.org/doc/html/rfc7519 und jwt.io wie das geht und nutze eine entsprechende Bibliothek !!SCHREIBE EINEN JWT-GENERATOR NICHT SELBER!! - Für PHP nutzen wir https://github.com/lcobucci/jwt aber jede Bibliothek die HS512 bzew SHA512 unterstützt ist möglich.  
  1. Nutze für den JWT HS512 bzw SHA512  
  1. Als key nutze secret  
  1. Schreibe in den Header als feld tokenversion den wert tokenversion  
  1. Als issued by (iss) setze den Origin der Seite (ohne / am ende)  
  1. Als audience (aud) setze https://eyesee.eyelearn.at  
  1. Setze issued at (iat) setze den jetzigen Zeitstempel (Aktuelle Unix Zeit)  
  1. Setze expires at (eat) auf jetzt +5min  
  1. Setze als Claim username den username.  
1. Speichere den JWT local as jwt  
1. Erzeuge eine URL wie folgt:  
  1. Origin ist https://eyesee.eyelearn.at  
  1. GET parameter sind:  
    1. action: externallogin  
    1. master: masterID  
    1. version: tokenversion  
    1. token: jwt  
1. Gebe url zurück  
...
1. Erzeuge das iframe mit  
  ...
  1. sandbox darf NICHT gesetzt werden  
